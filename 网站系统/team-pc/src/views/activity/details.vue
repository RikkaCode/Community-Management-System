<template>
    <div class="container">
        <!-- <el-image class="images" :src="image[0]" /> -->
        <div style="margin-left: 30px;">
            <strong>{{ title }}</strong>
            <div v-html="details"></div>
            <div>{{ createTime }}</div>
        </div>
    </div>
    <div v-if="hasJoin == '1'" style="
      display: flex;
      justify-content: center;
      align-items: center;
      margin: 60px 0px 100px 0px;
    ">
        <el-button @click="addCativity" icon="Edit" type="primary" size="default">报名活动</el-button>
    </div>
</template>

<script setup lang="ts">
    import { ref } from "vue";
    import { useRoute } from "vue-router";
    import { useRouter } from "vue-router";
    import { getNewsByIdApi, addActivityApi } from "@/api/news";
    import { userSotre } from "@/store/user";
    import useInstance from "@/hooks/useInstance";
    import { ElMessage } from "element-plus";

    const { global } = useInstance();
    const store = userSotre();
    const route = useRoute();
    const router = useRouter();
    console.log(route.params.id);
    // 标题
    const title = ref("");
    const id = ref("");
    const details = ref("0");
    const hasJoin = ref("");
    const image = ref([]);
    const createTime = ref("");

    // 根据id查询数据
    const getById = async () => {
        let res = await getNewsByIdApi(route.params.id as string);
        if (res && res.code == 200) {
            console.log(res);
            title.value = res.data.title;
            details.value = res.data.details;
            id.value = res.data.id;
            hasJoin.value = res.data.hasJoin;
            image.value = res.data.image.split(",");
            createTime.value = res.data.createTime;
        }
    };

    const addCativity = async () => {
        let cons = await global.$myconfirm("您确定参加该活动吗?");
        if (cons) {
            // 判断是否登录
            if (!store.userInfo.sutId) {
                let confirm = await global.$myconfirm("您未登录，请去登录");
                if (confirm) {
                    // 跳转登录
                    router.push({ name: "login" });
                }
            } else {
                let res = await addActivityApi({
                    activityId: id.value,
                    stuId: store.userInfo.sutId,
                });
                if (res && res.code == 200) {
                    ElMessage.success(res.msg);
                }
            }
        }
    };
    
    getById();
</script>

<style scoped>
    .container {
        display: flex;
        margin-top: 30px;
    }

    .images {
        height: 100px;
        width: 500px;
        border-radius: 10px;
    }
</style>